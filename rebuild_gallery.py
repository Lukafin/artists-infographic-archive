...
def classify_person(person: str) -> str:
    ...
    artist_terms = [
        ...
        'lady gaga', 'kurosawa', 'mussorgsky', 'van gogh', 'rafael', 'sanzio',
...
def render_featured(featured):
    ...
    category = classify_person(featured.get('person', ''))
  ...
  <div class="hero-copy surface">
    <div>
      ...
      <h2>Vsak dan nova infografika o znanih umetnikih in znanstvenikih.</h2>
      <p class="intro">Javni arhiv zbira dnevno ustvarjene slovenske infografike za otroke. Spodaj lahko hitro prelistaš starejše objave kot moderno galerijo slik.</p>
    </div>
    <div>
    </div>
  </div>
  <article class="hero-card surface">
    ...
    <div class="meta-row hero-meta">
      <span class="tag date">{date}</span>
      <span class="tag {'artist' if category == 'Umetnik' else 'science'}">{category}</span>
      <span class="tag source-count">{count_label}</span>
    </div>
    <h3>{person}</h3>
...
def render_page(page_num: int, total_pages: int, chunk, featured=None):
    ...
    page_intro = (
        'Starejše objave so prikazane kot image-first galerija za hitrejše brskanje.'
        if page_num == 1 else
        'Prelistaj starejše dnevne infografike po straneh.'
    )
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .pager.active {{ background:var(--accent); border-color:var(--accent); color:#fff; font-weight:700; }}
...
<body>
  <main class="wrap">
    ...
    <footer class="footer surface">
      <div>Javni arhiv dnevnih infografik o znanih umetnikih in znanstvenikih.</div>
